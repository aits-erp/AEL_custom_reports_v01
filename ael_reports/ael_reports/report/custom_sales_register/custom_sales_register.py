...
def _execute(filters, additional_table_columns=None):
	...
	# Fetch GST map for all invoices at once
	all_invoice_names = [inv.name for inv in invoice_list]
	gst_map_all = get_gst_tax_map_bulk(all_invoice_names)
	...
	for inv in invoice_list:
		...
		# Custom GST columns from bulk map
		gst_map = gst_map_all.get(inv.name, {})
		row.update({
			"cgst_2_5": flt(gst_map.get("cgst_2_5")),
			"sgst_2_5": flt(gst_map.get("sgst_2_5")),
			"igst_5":   flt(gst_map.get("igst_5")),
			"cgst_9":   flt(gst_map.get("cgst_9")),
			"sgst_9":   flt(gst_map.get("sgst_9")),
			"igst_18":  flt(gst_map.get("igst_18")),
		})
...
def get_columns(invoice_list, additional_table_columns, include_payments=False):
	...
	# Column order: income → tax accounts → GST columns → Net Total → unrealized → totals
	columns = (
		columns
		+ account_columns[0]
		+ account_columns[2]
