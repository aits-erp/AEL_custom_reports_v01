...
frappe.query_reports["Freight Margin Report"] = {
    ...
    formatter: function (value, row, column, data, default_formatter) {
        ...
        // ── child rows (items) ────────────────────────────────────────
        if (data && data.indent === 1) {
            // invoice column must render blank — it is a Link field
            if (column.fieldname === "invoice") return "";
            // all other child cells render muted
            return `<span style="color:var(--text-muted);font-size:0.92em;">${value || ""}</span>`;
        }

        // ── parent rows (invoices) ────────────────────────────────────
