...
def get_columns():
    return [
        ...
        {"label": "Gross Margin",        "fieldname": "gross_margin",       "fieldtype": "Currency", "width": 130},
        # ── NEW column ──────────────────────────────────────────────────
        {"label": "Gross Margin Total",  "fieldname": "gross_margin_total", "fieldtype": "Currency", "width": 150},
        # ────────────────────────────────────────────────────────────────
        {"label": "Gross Percent",       "fieldname": "gross_percentage",   "fieldtype": "Percent",  "width": 120},
        {"label": "Commission",          "fieldname": "commission",         "fieldtype": "Currency", "width": 120},
        {"label": "Net Margin",          "fieldname": "net_margin",         "fieldtype": "Currency", "width": 130},
...
def get_data(filters):
    ...
    for inv in invoices:
        ...
        if not po_list:
            ...
            inv_row["buy"]                = 0.0
            inv_row["gross_margin"]       = sell
            inv_row["gross_margin_total"] = sell
            inv_row["gross_percentage"]   = 100.0 if sell else 0
            inv_row["net_margin"]         = sell - commission
            result.append(inv_row)

            if total_amount_view == "Invoice Based":
                _append_children(result, inv, item_map, 0.0, sell, sell, commission)
            ...
            for po_name in po_list:
                ...
                inv_row["purchase_invoice"]   = pi_name
                inv_row["buy"]                = buy
                inv_row["gross_margin"]       = gm_row
                inv_row["gross_margin_total"] = gm_total   # same for all PO rows
                inv_row["gross_percentage"]   = gross_pct  # based on gm_total
                inv_row["commission"]         = commission
                inv_row["net_margin"]         = net_margin  # based on gm_total
                result.append(inv_row)

                if total_amount_view == "Invoice Based":
                    _append_children(result, inv, item_map, buy, sell, gm_total, commission)

    return result


def _append_children(result, inv, item_map, buy_total, sell_total, gm_total, commission):
    inv_items  = item_map.get(inv.invoice, [])
    item_count = len(inv_items) or 1
    ...
    for itm in inv_items:
        ...
        if sell_total:
            item_buy   = buy_total * (item_sell / sell_total)
            item_gm    = gm_total  * (item_sell / sell_total)
            item_comm  = commission * (item_sell / sell_total)
        else:
            item_buy   = buy_total  / item_count
            item_gm    = gm_total   / item_count
            item_comm  = commission / item_count

        item_gross     = item_sell - item_buy
        item_gross_pct = (item_gm / item_sell * 100) if item_sell else 0.0
        item_net       = item_gm - item_comm
